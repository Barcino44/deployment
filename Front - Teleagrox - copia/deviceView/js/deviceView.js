...
async function showTempMeasurements() {
	try {
		const response = await fetch(`http://localhost:8080/device/get/tempMeasurements/${deviceId}`);
...
async function showMoistureMeasurements() {
	try {
		const response = await fetch(`http://localhost:8080/device/get/moisMeasurements/${deviceId}`);
		if (!response.ok) throw new Error('Error al obtener mediciones');
		const measurements = await response.json();
