...
async function loadDeviceErrors() {
    ...
    let response = await fetch(`https://back-production-5d7a.up.railway.app:8080/admin/errors/${clientId}`, {
        method: 'GET',
        headers: {
...
async function deleteDevices(clientId, deviceIds) {
    ...
    try {
        const response = await fetch("https://back-production-5d7a.up.railway.app:8080/admin/clients/unlinkDevice", {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                idClient: clientId,
